...
export default function Transparency() {
  ...
  const docs = t('docs', { returnObjects: true }) as {
    token: string;
    token_url: string;
    termo: string;
  };
  ...
  return (
    <main className="bg-black text-white py-16 px-4">
      <section className="max-w-4xl mx-auto text-center space-y-6">
        <motion.h1 initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-5xl font-bold text-purple-400 drop-shadow-[0_0_25px_rgba(168,85,247,0.5)]">
          {t('title')}
        </motion.h1>
        <motion.p initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-lg text-gray-300">
          {t('intro')}
        </motion.p>
      </section>
      ...
      <section className="max-w-3xl mx-auto mt-24 grid gap-4 md:grid-cols-2">
        {Array.isArray(dna) &&
          dna.map((item, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="bg-gradient-to-br from-[#1e1b4b] to-[#111827] p-5 rounded-xl shadow-lg text-sm text-gray-200">
              {item}
            </motion.div>
          ))}
      </section>
      ...
      <section className="max-w-3xl mx-auto mt-20 text-center">
        <motion.h2 initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-3xl font-bold text-purple-300">
          {t('proof_title')}
        </motion.h2>
        <motion.p initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-base text-gray-400">
          {t('proof_intro')}
        </motion.p>
        <div className="mt-10 grid gap-4">
          {Array.isArray(proof) &&
            proof.map((item, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="bg-gradient-to-br from-[#1e1b4b] to-[#111827] p-5 rounded-xl text-sm text-gray-200">
                {item}
              </motion.div>
            ))}
        </div>
      ...
      <section className="max-w-3xl mx-auto mt-20 space-y-6 text-center">
        ...
        <div className="grid md:grid-cols-2 gap-6 text-left">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-[#1f1b2e] p-6 rounded-lg border border-green-600">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-[#1f1b2e] p-6 rounded-lg border border-red-600">
            <h4 className="text-lg font-bold text-red-400 mb-2">{t('governance.cannot.title')}</h4>
            <ul className="list-disc ml-4 space-y-2 text-sm text-gray-300">
              {Array.isArray(cannotItems) &&
      ...
      <section className="max-w-4xl mx-auto mt-20 text-center space-y-10">
        <motion.p initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-base text-gray-300">
          {t('protection.summary')}
        </motion.p>
        <motion.div initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-[#1f1b2e] p-6 rounded-lg border border-purple-600">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
          {Array.isArray(badges) &&
            badges.map((label, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="text-center">
...
<section className="max-w-4xl mx-auto mt-28 text-center space-y-6 text-sm text-gray-300">
  ...
  >
    {t('lock_reason.button')}
      ...
      <section className="max-w-5xl mx-auto mt-20 space-y-8">
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {wallets.map((item, idx) => {
            ...
            return (
              <li key={idx} className={`bg-[#1e1b4b] p-6 rounded-xl border border-purple-700 shadow-md text-sm text-center flex flex-col items-center justify-center ${shouldSpan ? 'md:col-span-3' : ''}`}>
                ...
                <a href={`https://bscscan.com/address/${item.address}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 underline text-xs mt-2 inline-block">
      ...
      <p className="text-xs">
  {t('docs_links.termo_label')}{' '}
  <a
    href="/contact"
    rel="noopener noreferrer"
    className="underline text-purple-400 hover:text-purple-300"
  >
    {t('docs_links.termo_link')}
  </a>
</p>
